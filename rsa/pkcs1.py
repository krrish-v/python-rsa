...
def encrypt(message: bytes, pub_key: tuple) -> bytes:
    ...
    keylength = common.byte_size(pub_key[0])
    padded = _pad_for_encryption(message, keylength)

    payload = transform.bytes2int(padded)
    encrypted = core.encrypt_int(payload, pub_key[1], pub_key[0])
    block = transform.int2bytes(encrypted, keylength)

    return block


def decrypt(crypto: bytes, priv_key: tuple) -> bytes:
    ...
    blocksize = common.byte_size(priv_key[0])
    encrypted = transform.bytes2int(crypto)
    decrypted = key.PrivateKey(*priv_key).blinded_decrypt(encrypted)
